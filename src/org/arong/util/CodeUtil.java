...
/**
 * 阿荣自创的加密算法（O(∩_∩)O~），从arong-1.2.0.jar中抽取出来，配合数据库连接信息的加密使用。
 * 
 * @author 阿荣
 * 
 */
public final class CodeUtil {
	 ...
	 */
	public static String myDecode(String codeStr) throws NumberFormatException {
		String str = null;
		// 待反编码的字符串不为空
		if (codeStr != null && codeStr.trim().length() > 0) {
			...
			for (int i = 1; i < codes.length - 1; i++) {
				code = Integer.parseInt(codes[i]) - DIS1 - i + 1 - DIS2 - start
						+ end;
				sb.append((char) code);
			}
			str = sb.toString();
